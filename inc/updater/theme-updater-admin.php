...
class EDD_Theme_Updater_Admin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'chique_about_after', array( $this, 'license_page' ) );
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'chique-pro' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							esc_html__( 'Your license key expired on %s.', 'chique-pro' ),
					...
					case 'revoked' :
						...
						$message = esc_html__( 'Your license key has been disabled.', 'chique-pro' );
						break;

					case 'missing' :

						$message = esc_html__( 'Invalid license.', 'chique-pro' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = esc_html__( 'Your license is not active for this URL.', 'chique-pro' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'chique-pro' ), $this->item_name );
						break;

					case 'no_activations_left':

						$message = esc_html__( 'Your license key has reached its activation limit.', 'chique-pro' );
						break;
		...
		wp_redirect( admin_url( add_query_arg( array( 'page' => 'chique-about', 'tab' => 'license' ), 'themes.php' ) ) );
		exit();
...
add_action( 'admin_notices', 'chique_admin_notices' );
